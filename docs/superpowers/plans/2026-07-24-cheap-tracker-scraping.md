...
```bash
rm -f scratch/fetch_reports/*.json
# Targeted, not `git add -A`: the working tree carries unrelated in-progress
# edits to README.md and scripts/generate_readme.py from an earlier session
# that must not be swept into a scrape commit.
git add data/ sources/scrape_state.yaml
...
Note: `README.md` is regenerated by the merge step, but it also carries an
uncommitted formatting change from an earlier session. Inspect `git diff
README.md` and stage it only if both changes are intended.
